...
function Generate_Certificates {
	...
	if [ -z $ROOT_COMMON_NAME ]; then
		ROOT_COMMON_NAME=$(dig +short myip.opendns.com @resolver1.opendns.com)
	fi
	sed -i "s/###IP###/$ROOT_COMMON_NAME/g" /etc/ssl/openssl.cnf
	...
	openssl req -x509 -new -passin pass:${PASS_ROOT} -subj "/C=${COUNTRY}/ST=${STATE}/L=${LOCALITY}/O=${ORGANIZATION}/OU=${UNIT}/CN=${ROOT_COMMON_NAME}/emailAddress=${EMAIL}/SAN=*.${ROOT_COMMON_NAME}.xip.io" -nodes -key devdockerCA.key -days 10000 -out devdockerCA.crt
	# Generate a key for server
	openssl genrsa -out domain.key 2048
	# Make a certificate signing request
	openssl req -new -passin pass:${PASS_ROOT} -subj "/C=${COUNTRY}/ST=${STATE}/L=${LOCALITY}/O=${ORGANIZATION}/OU=${UNIT}/CN=${ROOT_COMMON_NAME}/emailAddress=${EMAIL}/SAN=*.${ROOT_COMMON_NAME}.xip.io" -key domain.key -out dev-docker-registry.csr
